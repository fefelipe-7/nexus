...
import { Flame, TrendingUp, TrendingDown, Minus, CheckCircle, Pause, Activity } from 'lucide-react';
import { cn } from '@nexus/shared';
import type { HabitsSummary } from '../../types/habits.types';

interface HabitsHeaderProps {
    summary: HabitsSummary;
}

export function HabitsHeader({ summary }: HabitsHeaderProps) {
    const getTrendIcon = () => {
            ...
            <div className="p-4 bg-gradient-to-br from-primary/5 via-transparent to-transparent">
                ...
                <div className="grid grid-cols-4 gap-2">
                    ...
                    <div className="bg-background/60 rounded-xl p-2.5 text-center">
                        ...
                        <p className="text-sm font-bold">{summary.activeStreaks}</p>
                        <p className="text-[9px] text-muted-foreground">Sequências</p>
                    </div>
                    <div className="bg-background/60 rounded-xl p-2.5 text-center">
                        <CheckCircle className="h-4 w-4 mx-auto text-green-500 mb-1" />
                        <p className="text-sm font-bold">{summary.todayCompleted}/{summary.todayTotal}</p>
                        <p className="text-[9px] text-muted-foreground">Hoje</p>
                    </div>
                    <div className={cn(
                        "rounded-xl p-2.5 text-center",
                        summary.pausedCount > 0 ? "bg-purple-500/10" : "bg-background/60"
                    )}>
                        <Pause className={cn(
                            "h-4 w-4 mx-auto mb-1",
                            summary.pausedCount > 0 ? "text-purple-500" : "text-muted-foreground"
                        )} />
                        <p className="text-sm font-bold">{summary.pausedCount}</p>
                        <p className="text-[9px] text-muted-foreground">Pausados</p>
                    </div>
                </div>
            </div>
        </Card>
